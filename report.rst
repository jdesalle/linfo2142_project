...
We didn't measure the app_connect time since there seem to be an ongoing issue in curl with it on http3 [#]_
...
After capturing the traffic from the client side and from the server side, we created the .qlog files analyzed them using another tool : *qvis* [#]_ .
